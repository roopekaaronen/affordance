...
pB <- ggplot(data = aggB) +
  geom_point(aes(pro.amount, pro.behavior, colour="Proenvironmental"), alpha = 0.1) +
  geom_smooth(aes(pro.amount, pro.behavior), se = FALSE, method = "loess", show.legend = FALSE, colour = "gray20", size = 1.2, span=0.15) +
  geom_point(aes(pro.amount, non.behavior, colour="Nonenvironmental"), alpha = 0.1) + 
  geom_smooth(aes(pro.amount, non.behavior), se = F, method = "loess", show.legend = FALSE, colour = "firebrick3", size = 1.2, span=0.15) +
...
## READ DATA
dataT = read.csv("H:/MyDocuments/Draft/Analysis/CSV/Stylized.timeseries.csv", stringsAsFactors = FALSE)
...
pn <- ggplot(data=means.neutral) + 
  ...
  geom_smooth(aes(x=X.step., y=meanpro), color = "gray20", se = FALSE) +
#  geom_smooth(data=neutral, aes(group=X.run.number., x=X.step., y=pro.behavior), se = FALSE, color = "gray20") + # Remove hashtag to view all individual lines.
  ...
  ggtitle("Time-series of pro-environmental behavior, 50% pro-environmental affordances", subtitle = 
...
pb <- ggplot() + 
  geom_smooth(data=means.bias, aes(x=X.step., y=meanpro), color = "gray20", se = FALSE) +
...
#  geom_smooth(data=bias, aes(group=X.run.number., x=X.step., y=pro.behavior), color = "gray20", se = FALSE) + # Remove hashtag to view all individual lines.
  ...
  ggtitle("Time-series of pro-environmental behavior, 60% pro-environmental affordances", subtitle = 
            "300 runs with niche construction") +
...
pb2 <- ggplot() + 
  geom_smooth(data=means.bias2, aes(x=X.step., y=meanpro), color = "gray20", se = FALSE) +
...
pBC <- ggplot(dataBC) + 
  geom_line(aes(x=Year, y=Vehicles..thousand., col = "Vehicles"), fill = "gray20", size = 1) +
  geom_line(aes(x=Year, y=Bicycles..thousand., col = "Bicycles"), fill = "firebrick3", size = 1) +
  xlab("Year") +
...
cphplot <- ggplot(cphbike) +
  geom_point(aes(year, cphbike)) +
  geom_smooth(aes(year, cphbike), se = FALSE, color = "grey20", method = "lm") +
  xlab("Year") +
...
# READ DATA
dataC = read.csv("H:/MyDocuments/Draft/Analysis/CSV/Copenhagen.timeseries300.csv", skip = 6, stringsAsFactors = FALSE)
...
p <- ggplot(data.means) + 
  geom_smooth(aes(y=meanpro, x=X.step., colour="Bicycle"))+
  geom_ribbon(aes(ymin=meanpro-sdpro, ymax=meanpro+sdpro, x=X.step., fill = "band"), alpha = 0.2, fill = "gray20")+
  geom_smooth(aes(y=meannon, x=X.step., colour="Vehicle"))+
  geom_ribbon(aes(ymin=meannon-sdnon, ymax=meannon+sdnon, x=X.step., fill = "band"), alpha = 0.2, fill = "firebrick3")+
  ylim(-0.02, 1.02) +
  xlim(1970, 2028) +
  ggtitle("Pro-environmental and non-environmental behavior over time (300 runs)") +
  xlab("Year") +
...
# PLOT THE SINGLE RUN
ps <- ggplot(single) + 
  geom_smooth(aes(y=pro.behavior, x=X.step., colour="Bicycle"))+
  geom_smooth(aes(y=non.behavior, x=X.step., colour="Vehicle"))+
  geom_line(aes(y=pro.behavior, x=X.step., colour="Bicycle"), alpha = 0.5)+
  geom_line(aes(y=non.behavior, x=X.step., colour="Vehicle"), alpha = 0.5)+
  ylim(-0.02, 1.02) +
  xlim(1970, 2028) +
  ggtitle("Pro-environmental and non-environmental behavior over time (selected single run)") +
  xlab("Year") +
...
pA <- ggplot(data.means) + 
  geom_smooth(aes(y=meanaff, x=X.step.), color = "grey20")+
...
pTB <- ggplot(data = aggTB, aes(x = construct.pro)) +
  geom_point(aes(construct.pro, pro.behavior, colour="Proenvironmental"), alpha = 0.2) +
  geom_point(aes(construct.pro, non.behavior, colour="Nonenvironmental"), alpha = 0.2) +
  geom_smooth(aes(construct.pro, pro.behavior, colour="Proenvironmental"), se = FALSE, method = "auto", span = 0.9, show.legend = F, colour = "gray20", size = 1.2) +
  geom_smooth(aes(construct.pro, non.behavior, colour="Nonenvironmental"), se = F, method = "auto", span = 0.9, show.legend = F, colour = "firebrick3", size = 1.2) +
...
fviz_cluster(object = k.means.fit, data = mydata.stand, geom = "point", choose.vars = c("pro.amount", "pro.behavior"), ellipse.type = "norm", ellipse.level
= 0.95) + theme_bw() + ggtitle("Clusters (95% ellipses) of pro-environmental behavior by amount of affordances,
                               without niche construction")
...
degreeplot1 <- ggplot(net, aes(x = net)) +
  ...
  scale_x_continuous(breaks = round(seq(min(net$net), max(net$net), by = 5),1)) +
  xlab("Number of links") +
  ylab("Frequency") +
  ggtitle("Network degree distribution") +
  geom_vline(xintercept=mean(net$net), lwd=1, linetype=2, color="grey20") +
  geom_vline(xintercept=median(net$net), lwd=1, linetype=2, color="firebrick3")
...
degreeplot2 <- ggplot(test2, aes(x = test)) +
  ...
  ggtitle("Network degree distribution (1000 simulations / 300,000 agents)") +
  geom_vline(xintercept=mean(net$net), lwd=1, linetype=2, color="gray20")  +
  geom_vline(xintercept=median(net$net), lwd=1, linetype=2, color="firebrick3")  +
...
clustercoeff <- ggplot(dataN, aes(x = dataN$global.clustering.coefficient)) +
  ...
  geom_vline(xintercept=mean(dataN$global.clustering.coefficient), lwd=1, linetype=2, color="black")
...
g <- ggplot(data = senpop, aes(y = pro.behavior, x = number.of.agents, fill = number.of.agents)) +
  ...
  geom_point(aes(y = pro.behavior, color = number.of.agents), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
...
sumld<- ddply(senpop, ~network.param, summarise, mean = mean(pro.behavior), median = median(pro.behavior), lower = lb(pro.behavior), upper = ub(pro.behavior))

g2 <- ggplot(data = sennet, aes(y = pro.behavior, x = network.param, fill = network.param)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = pro.behavior, color = network.param), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
  expand_limits(x = 5.25) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
...
sumld<- ddply(senpop, ~social.learning, summarise, mean = mean(pro.behavior), median = median(pro.behavior), lower = lb(pro.behavior), upper = ub(pro.behavior))

g3 <- ggplot(data =sensoc, aes(y = pro.behavior, x = social.learning, fill = social.learning)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = pro.behavior, color = social.learning), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
  expand_limits(x = 5.25) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
...
sumld<- ddply(senpop, ~pro.amount, summarise, mean = mean(pro.behavior), median = median(pro.behavior), lower = lb(pro.behavior), upper = ub(pro.behavior))

g4 <- ggplot(data =senenv, aes(y = pro.behavior, x = pro.amount, fill = pro.amount)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = pro.behavior, color = pro.amount), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
  expand_limits(x = 5.25) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
  ...
  xlab("Proportion of pro-environmental affordance") +
  ylab("Proportion of pro-environmental behavior") +
  ggtitle("Sensitivity testing for proportion of affordances (output at step 2000)")
...
sumld<- ddply(senpop, ~construct.pro, summarise, mean = mean(pro.behavior), median = median(pro.behavior), lower = lb(pro.behavior), upper = ub(pro.behavior))

g5 <- ggplot(data =sennc, aes(y = pro.behavior, x = construct.pro, fill = construct.pro)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = pro.behavior, color = construct.pro), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
  expand_limits(x = 5.25) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
...
sumld<- ddply(senpop, ~asocial.learning, summarise, mean = mean(pro.behavior), median = median(pro.behavior), lower = lb(pro.behavior), upper = ub(pro.behavior))

g6 <- ggplot(data =senasoc, aes(y = pro.behavior, x = asocial.learning, fill = asocial.learning)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = pro.behavior, color = asocial.learning), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
  expand_limits(x = 5.25) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
...
sumld<- ddply(senpop, ~initial.pro, summarise, mean = mean(pro.behavior), median = median(pro.behavior), lower = lb(pro.behavior), upper = ub(pro.behavior))

g7 <- ggplot(data =senpro, aes(y = pro.behavior, x = initial.pro, fill = initial.pro)) +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .8) +
  geom_point(aes(y = pro.behavior, color = initial.pro), position = position_jitter(width = .15), size = .5, alpha = 0.8) +
  geom_boxplot(width = .1, guides = FALSE, outlier.shape = NA, alpha = 0.8) +
  expand_limits(x = 5.25) +
  guides(fill = FALSE) +
  guides(color = FALSE) +
...
modelpath <- file.path("H:/MyDocuments/Draft/Model/behaviour-field7.nlogo")
outpath <- file.path("H:/MyDocuments/Draft/Analysis/NLRX")
...
nl@experiment <- experiment(expname="affordance",
                            outpath="H:/MyDocuments/Draft/Analysis/NLRX",
                            repetition=1,
                            tickmetrics="false",
                            idsetup="setup",
                            idgo="go",
...
w <- ggplot(results.sum) +
  geom_point(data = results, aes(siminputrow, pro.behavior), alpha = 0.8, color = "gray20") +
geom_errorbar(data = results.sum, aes(siminputrow, ymin = min, ymax=  max), width=0.5, colour = "gray20") +
...
pra <- ggplot(results) + 
  geom_point(aes(y=pro.behavior, x=pro.amount), alpha = 0.8)+
...
prba <- ggplot(results) + 
  geom_point(aes(y=pro.behavior, x=initial.pro), color = "grey20", alpha = 0.8)+
 # geom_smooth(aes(y=pro.behavior, x=initial.pro, color = "initial.pro"), method='lm',formula=y~x)+
  ...
  geom_rect(aes(xmin = 0.55, xmax = 0.66, ymin = 0.05, ymax = 0.25),
            fill = "transparent", color = "firebrick3", size = 1)
